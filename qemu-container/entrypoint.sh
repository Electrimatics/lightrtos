...
if [ $START_TELNET -ne 0 ]; then
    lxterminal --title "QEMU USART" -e "until kill -0 $(pidof qemu-system-avr); do : ; done; telnet localhost 10000;" &
fi

if [ $START_DEBUG -ne 0 ]; then
    # TODO: Add ability to pass GDB file in
    QEMU_RUN_ARGS+="--gdb tcp::10001 -S"
    lxterminal --title "QEMU DEBUG" -e "until kill -0 $(pidof qemu-system-avr); do : ; done; \
